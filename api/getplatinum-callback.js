...
export default async function handler(req, res) {
  ...
  if (notificationType === 1 && isSuccess) {
    ...
    const dialogId = payment.dialog_id
    const vkUserId = payment.vk_user_id
    ...
    if ([2, 3].includes(productId)) {
      try {
        ...
        await trace(traceId, 'getplatinum.tarot_callback_failed', {
          deal_id: dealId,
          dialog_id: dialogId,
          vk_user_id: vkUserId,
        ...
        return res.status(200).json({ ok: false, reason: 'tarot_setup_failed' })
      }
    }
    ...
    const { error: dialogUpdErr } = await supabase
      ...
      .update({
        ...
        product_step: 0,
        current_reading_id: [2, 3].includes(productId) ? tarotReadingId : null,
        cycle_started_at: null,
        cycle_completed_at: null,
        next_action_at: [2, 3].includes(productId) ? tarotNextActionAt : null,
        last_message_at: now,
        last_message_by: 'bot',
    ...
    const reply = [2, 3].includes(productId)
      ? (offerName
    ...
    try {
      const result = await sendMessage(vkUserId, reply)
